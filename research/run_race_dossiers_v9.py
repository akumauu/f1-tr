...
DEFAULT_CONFIG = (
    ROOT / "research" / "configs" / "race_dossier_2023_2025_v9.json"
)
...
def main() -> int:
    ...
    if config.get("method_version") != "v9":
        parser.error("v9 runner 只接受 method_version=v9")
    result = engine.run(
        config,
        run_id=args.run_id,
